...
std::vector<std::byte> VideoCoder::encode(Video & video) {
    std::vector<std::byte> bytes = {};

    // append header
    const std::byte* header_byte_pointer = reinterpret_cast<const std::byte*>(&video.header);
    bytes.insert(bytes.end(), header_byte_pointer, header_byte_pointer + sizeof(video.header));

    // append frames
    for (auto & frame : video.frames) {
        const auto frame_pixels_pointer = reinterpret_cast<std::byte*>(frame.pixels.data());
        bytes.insert(
                bytes.end(), 
                frame_pixels_pointer, 
                frame_pixels_pointer + frame.pixels.size());
    }
    return bytes;
}

Video VideoCoder::decode(std::vector<std::byte> & bytes) {
    Video video = {};
...
int VideoCoder::getBytesPerFrame(const VideoHeader & video_header) {
